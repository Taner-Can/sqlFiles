...
?
--What are the differences between TRUNCATE and DELETE? (Interview)
...
SELECT *
FROM workers
WHERE salary IN (2000, 7000, 12000); -- Better way with "IN" from inside the list.
?
--How to select specific cell ( just to get 'Veli Han' )
SELECT name
FROM workers
WHERE id=10002;
?
...
SELECT MAX(salary) AS maximum_salary  --By using "AS" you can put temporary field names for a more readible table on the console
FROM workers;
